...
class ViewController: UIViewController {
    ...
    private func makeRandomNumber() -> [Int] {
        // random으로 숫자를 얻어내서 secrete 번호 3개 알아내기
    ...
    /* 결과 확인
     #룰 : S는 결과에 포함되고 자릿수 가 같음
     B은 결과에 포함은 됐지만 자릿수가 같지 않음
     O은 결과에 포함도 되지 않음
     #로직 : 레이블의 숫자를 하나씩 가져와서 결과에 포함되어 있는지 확인, 그리고 자릿수가 같은지 다시한번 확인
    */
    private func checkAnswer(answer:[Int], correctAnswer:[Int]) -> String {
        ...
        */
        ...
        let answerList: [Int] = answer
        let correctAnswerList: [Int] = correctAnswer
    ...
    @IBOutlet var selectNum1LB: UILabel!
    @IBOutlet var selectNum2LB: UILabel!
    @IBOutlet var selectNum3LB: UILabel!
    ...
    /*
     2. 숫자 버튼 클릭
     - 결과 레이블에 추가
     - 레이블이 각각 다른 인스턴스 인데 어떻게 추가되어야되나?
     - 중복 숫자 입력 방지
     */
    // 숫자 입력
    ...
    override func viewDidLoad() {
        ...
        selectNumLBList = [selectNum1LB, selectNum2LB, selectNum3LB]
    }
