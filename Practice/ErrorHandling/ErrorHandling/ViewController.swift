...
final class ViewController: UIViewController {
  ...
  /********************************
   ...
   - throws 키워드 주목
   - write(to: caches, automically: true, encoding: .utf8) // 컴파일 에러
   error - Call can throw, but it is not marked with 'try' and the error i not handled
   ********************************/
  ...
  override func viewDidLoad() {
    ...
    //basic()
    //throwsTypesOfTry()
    //throwFunctionExample()
    // rethrowFunctionExample()
  ...
  private func basic() {
    ...
    let swift = "Swift Error Handling"
    do {
      //try swift.write(toFile: tmpDir, atomically: true, encoding: .utf8)  // Error (tmpDir is Directiory)
      try swift.write(toFile: filePath, atomically: true, encoding: .utf8)  // No Error
    } catch /*let e* error == e*/ {   // <- catch 만 입력 시 기본적으로 let error 를 사용한 것과 동일
      print(error)
      print(error.localizedDescription)
    }
    
    /******************************************
     do ~ catch q 블럭에서 위와 같이 첫 번째 try 문에서 에러가 발생하면 해당 코드 이후의 내용들은 실행되지 않음.
     즉 Error가 나지 않는 두 번째 try 구문은 실행되지 않으므로 주의 필요
     ****************************************/
  }
  
  // MARK: - try type
  
  private func threeTypeOfTry() {
    print("\n----------------[ try types ]------------------\n")
    /******************************************
     try vs try? vs try!
     ****************************************/
...
//      _ = try! String(contentsOfFile: errorFilePath)  // do ~ catch 문이어도 Crash 발생
  ...
  private func throwNSError() throws {
    ...
    // init(domain: String, code: Int, userInfo dict: [Stirng : Any]? = nil
    throw NSError(domain: "domain data", code: 55, userInfo: ["something": 1])
  }
  
  private func throwError() throws {
    // throws 메서드에서는 throw를 통해 직접 에러를 던지거나
...
//    }
  ...
  private func deferExample() {
    print("\n--------------[ deferExample ]----------------\n")
    defer { print("task1 - Must do something ")
    }
    do {
      try throwError()
    } catch {
    ...
    defer {
      print("THis is not called when error occeurred")
    }
  }
