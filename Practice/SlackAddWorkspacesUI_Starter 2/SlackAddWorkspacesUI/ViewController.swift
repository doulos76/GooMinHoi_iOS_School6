...
//                         constant: 0).isActive = true
  ...
  private func underlineStyleByProgrammaticallyExample() {
    ...
    let mutableAttStr = NSMutableAttributedString(string: buttonTitle)
    let attrKeys: [NSAttributedStringKey: Any] = [.underlineStyle: NSUnderlineStyle.styleSingle.rawValue, .underlineColor: UIColor.darkGray]
    mutableAttStr.addAttributes(attrKeys, range: NSRange(location: 0, length: buttonTitle.count))
    nextButton.setAttributedTitle(mutableAttStr, for: .normal)
  }
